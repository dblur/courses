...
private String printTextPerRole(String[] roles, String[] textLines) {
    StringBuilder replics = new StringBuilder();
    for (String role : roles) {
        replics.append(role + ":").append("\n");
        for (int i = 0; i < textLines.length; i++) {
            if (textLines[i].startsWith(role + ":")) {
                replics.append(i + 1).append(")").append(textLines[i].substring(role.length() + 1)).append("\n");
            }
         }
     replics.append("\n");
                }
    return replics.toString();
                }
